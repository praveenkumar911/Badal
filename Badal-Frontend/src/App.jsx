...
function App() {
    ...
    const tokenChangeListener = (event) => {
      if (event.key === 'token') {
        ...
        if (newToken) {
          ...
          // Handle logout if token is removed
          handleLogout();
        }
      }
    };
  ...
  const fetchUserData = async (token) => {
    try {
      ...
      const dataResponse = await fetch('http://10.8.0.13:5030/get-permissions', {
        headers: {
  ...
  const handleLogin = async (formData) => {
    try {
      const response = await fetch('http://10.8.0.13:5030/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Store the user token in sessionStorage and localStorage
