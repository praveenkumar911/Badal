...
import EditTeam from "../Projects/editteam";
const Groups = () => {
  ...
  useEffect(() => {
    if (teams.length > 0) { 
      ...
      axios.get('http://10.8.0.13:5030/get-img-url', { params: { orgIds: teams.map(team => team.orgId) } })
        .then(response => {
  ...
  useEffect(() => {
    if (teams && teams.length > 0) {
      ...
      if (userIds.length > 0) {
        axios.post('http://10.8.0.13:5030/get-usernames', { userIds })
          .then(response => {
            ...
            // Update the usernames state with the fetched usernames
  ...
  useEffect(() => {
    // Fetch all teams data 
    axios.get('http://10.8.0.13:5030/get-all-teams')
      .then(response => {
...
<div>
  ...
  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "40px", }}>
    ...
    Active Projects {team.assignedModules.length}
          ...
          <div style={{display: "flex",justifyContent: "center",alignItems: "center"}}>
           {/*  <img src={notes} alt="Notes" style={{ width: '48px', height: '48px', marginTop: '-5px',cursor:'pointer',color: "#F6F4F4" }} /> */}
            <img src={gitlab} alt="GitLab" style={{ width: '48px', height: '48px', marginTop: '-5px',cursor:'pointer' }}  onClick={() => {
                      ...
                      {(isP005Allowed ||isP001Allowed) && (
             <img src={notes} alt="Notes" style={{ width: '48px', height: '48px', marginTop: '-5px',cursor:'pointer' }} onClick={() => handleOpen2(team.teamId)}/>
                      )}
          </div>
        </Grid>
      </Grid> )}
