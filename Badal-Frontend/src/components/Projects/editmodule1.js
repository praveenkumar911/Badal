...
function EditModule1(props) {
  ...
  useEffect(() => {
    const fetchModuleDetails = async () => {
      try {
        const res = await axios.get(`http://10.8.0.13:5030/modules/${moduleID}`);
        setUsefulData({
            ...res.data,
  ...
  const handleEditModule = async () => {
    setIsLoading(true);
    try {
      const { moduleDescription,gitlabId } = usefulData;
      const repoGitlabId = gitlabId || usefulData.forkedGitlabId;
      const gitlabProjectRes = await axios.put(`http://10.8.0.13:5030/edit-repo/${repoGitlabId}`, {
        //name: projectName,
      ...
      if (selectedFile) { 
        try {
          ...
          await axios.post(`http://10.8.0.13:5030/push-to-folder/${repoGitlabId}/Requirements`, pushToFileFolderData);
      ...
      if (selectedFile1) {
        try {
          ...
          await axios.post(`http://10.8.0.13:5030/push-to-folder/${repoGitlabId}}/UI Mocks`, pushToFileFolderData);
      ...
      if (selectedFile2) {
        try {
          ...
          await axios.post(`http://10.8.0.13:5030/push-to-folder/${repoGitlabId}/Database Documentation`, pushToFileFolderData);
      ...
      if (selectedFile3) {
        try {
          ...
          await axios.post(`http://10.8.0.13:5030/push-to-folder/${repoGitlabId}/API Documentation`, pushToFileFolderData);
      ...
      if (selectedFile4) {
        try {
          ...
          await axios.post(`http://10.8.0.13:5030/push-to-folder/${repoGitlabId}/Others`, pushToFileFolderData);
      ...
      const res = await axios.put(`http://10.8.0.13:5030/edit-moduleDB/${moduleID}`, usefulData);
      console.log('Module updated:', res.data);
      const logData = {
        index: 'badal',
        data: {
          message: `Edited module ${usefulData.moduleName}`,
          timestamp: new Date(),
        },
      };
      ...
      const logData = {
        index: 'badal',
        data: {
          message: `Error editing module ${usefulData.moduleName}: ${error.message}`,
          timestamp: new Date(),
        },
      };
      await axios.post('http://10.8.0.13:5030/log', logData);
