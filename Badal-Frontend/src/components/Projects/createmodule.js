...
useEffect(() => {
  const fetchSkills = async () => {
    try {
      const res = await axios.get(`http://10.8.0.13:5030/get-project/${projectID}`);
...
if (endDate <= usefulData.moduleDateStart) {
  ...
  return;
      ...
      // Validate Skills (You may customize this validation based on your requirements)
        ...
        const createRepoFolderRes = await axios.post(`http://10.8.0.13:5030/create-repo-folder/${GitGroupID}`, createRepoFolderData);
        ...
        const res = await axios.post('http://10.8.0.13:5030/create-module-DB', { ...usefulData, GitlabID, Gitprojectname, GitWebUrl,  requirementsDocument,
        uiMocks,
        apiDocument,
        ...
        const response = await axios.post(`http://10.8.0.13:5030/add-module-ids/${usefulData.projectid}`, { moduleIds: moduleId });
        ...
        if (selectedFile) {
          try {
            ...
            await axios.post(`http://10.8.0.13:5030/push-to-folder/${GitlabID}/Requirements`, pushToFileFolderData);
        ...
        if (selectedFile1) {
          try {
            ...
            await axios.post(`http://10.8.0.13:5030/push-to-folder/${GitlabID}/UI Mocks`, pushToFileFolderData);
        ...
        if (selectedFile2) {
          try {
            ...
            await axios.post(`http://10.8.0.13:5030/push-to-folder/${GitlabID}/Database Documentation`, pushToFileFolderData);
        ...
        if (selectedFile3) {
          try {
            ...
            await axios.post(`http://10.8.0.13:5030/push-to-folder/${GitlabID}/API Documentation`, pushToFileFolderData);
        ...
        if (selectedFile4) {
          try {
            ...
            await axios.post(`http://10.8.0.13:5030/push-to-folder/${GitlabID}/Others`, pushToFileFolderData);
