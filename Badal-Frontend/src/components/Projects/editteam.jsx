...
function EditTeam(props) {
  ...
  useEffect(() => {
    const fetchTeamDetails = async () => {
      try {
        const response = await axios.get(`http://10.8.0.13:5030/get-team-by-id/${teamId}`);
        ...
        const usernamesResponse = await axios.post('http://10.8.0.13:5030/get-usernames', { userIds });
  ...
  useEffect(() => {
    const fetchNgos = async () => {
      try {
        if (isP001Allowed) {
          const res = await axios.get(`http://10.8.0.13:5030/get-org/${orgId}`);
          setNgosList(res.data);
        } else if (isP005Allowed) {
          const res = await axios.get(`http://10.8.0.13:5030/org/role2`);
          setNgosList(res.data);
        } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (isP001Allowed) {
          const permissionsResponse = await axios.get('http://10.8.0.13:5030/get-extra-permissions');
          ...
          if (permissions && Array.isArray(permissions) && permissions.length >= 2) {
            ...
            const teamMembersResponse = await axios.get(`http://10.8.0.13:5030/get-users-by-org-for-team/${orgId}/${teamSize}`);
            setTeamMembers(teamMembersResponse.data);
          } else {
            console.error('Invalid or missing permissions data.');
          } 

          const orgDetailsResponse = await axios.get(`http://10.8.0.13:5030/organizationbyid/${orgId}`);
          ...
          if (permissions && Array.isArray(permissions) && permissions.length >= 2) {
            const teamSize = permissions[1].count;
            const teamMembersResponse = await axios.get(`http://10.8.0.13:5030/get-users-by-org-for-team/${projectOrg}/${teamSize}`);
            setTeamMembers(teamMembersResponse.data);
          } else {
            console.error('Invalid or missing permissions data.');
          }

          const orgDetailsResponse = await axios.get(`http://10.8.0.13:5030/organizationbyid/${projectOrg}`);
          setOrgGitID(orgDetailsResponse.data['gitlabId']);
        } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post('http://10.8.0.13:5030/get-usergitids', { userIds });
      ...
      const subgroupResponse = await axios.put(`http://10.8.0.13:5030/update-subgroup/${subgroupid}`, {
      ...
      if (subgroupResponse.status === 200) {
        ...
        const teamUpdateResponse = await axios.put(`http://10.8.0.13:5030/update-team/${teamId}`, {
          teamName,
          userIds: userIds,
        ...
        if (teamUpdateResponse.status === 200) {
          ...
          const updateTeamUserResponse = await axios.put('http://10.8.0.13:5030/update-teamuser', {
          ...
          if (updateTeamUserResponse.status === 200) {
            ...
            const deleteTeamUserResponse = await axios.put('http://10.8.0.13:5030/delete-teamuser', {
