...
const ModifiedCard = (props) => { 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch project data
        const projectDataResponse = await axios.post("http://10.8.0.13:5030/get-projectdata", { projectName: project });
        ...
        const orgDataResponse = await axios.get(`http://10.8.0.13:5030/get-org/${projectData[0].projectOwner}`);
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`http://10.8.0.13:5030/delete-group/${usefulData.gitlabId }`);
      console.log(response.data);
  
      // Delete project in database
      await axios.delete(`http://10.8.0.13:5030/delete-project/${projectId}`);
      console.log(`Project with ID ${projectId} deleted from database`);
  
      // Delete project ID from NGO collection
      await axios.delete(`http://10.8.0.13:5030/delete-project-from-ngo/${projectId}`);
