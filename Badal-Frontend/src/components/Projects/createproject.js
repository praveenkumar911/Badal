...
function Createproject() {
 ...
 /*  useEffect(() => { 
  ...
  const fetchUsersByOrg = async (projectOwner) => {
    try {
      const res = await axios.get(`http://10.8.0.13:5030/get-users-by-org/${projectOwner}`);
  ...
  const handleCreateProject = async () => {
    ...
    switch (projectField) {
      ...
      const createProjectRes = await axios.post(`http://10.8.0.13:5030/create-subgroup/${GitGroupID}`, {
        name:projectName,
        description: `BOT: Project created as  [**${usefulData.projectName}**](http://10.8.0.13:3000/module/${usefulData.projectName}) \n \n \n${usefulData.projectDescription.slice(0, 125)}...`, 
        //groupId:GitGroupID,
      });
      ...
      await axios.post('http://10.8.0.13:5030/update-project-org', {
        orgId: usefulData.projectOwner,
        projectId: projectId,
      });
      const updateProjectManagerRes = await axios.put(`http://10.8.0.13:5030/update-role/${usefulData.projectManager}`, {
      roleId: 'R002-B',
    });
