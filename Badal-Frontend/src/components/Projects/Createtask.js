...
function CreateTask(props) {
  ...
  setIsLoading(true);
    ...
    let gitlabIssueUrl = '';
    try { 
      const response = await axios.post(`http://10.8.0.13:5030/create-issue/${moduledata[0].forkedGitlabId}`, {
    ...
    let newTask = null;
    try {
      const response = await fetch('http://10.8.0.13:5030/create-task-DB', {
        method: 'POST',
        headers: {
    ...
    try {
      const response = await fetch(`http://10.8.0.13:5030/module/${moduledata[0]._id}/tasks`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
