...
function Modulecard(props) {
  ...
  useEffect(() => {
    const fetchTeamDetails = async () => {
      try {
        const response = await axios.get('http://10.8.0.13:5030/get-all-teams');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://10.8.0.13:5030/get-extra-permissions');
  ...
  const handleModuleChange = async (event) => {
    ...
    if (selectedModule) {
      ...
      if (teamIndex !== -1 && teamIndex < selectedModule.workspaceIds.length) {
        ...
        try {
          const response = await axios.post("http://10.8.0.13:5030/get-task-DB-byworkspaceID", {
  ...
  /* const handleCircleClick = async (details) => {
    try {
      ...
      if (isP001Allowed) {
        const teamsResponse = await axios.get(`http://10.8.0.13:5030/get-org-teams/${orgId}`);
          ...
          if (selectedTeam) {
            ...
            if (selectedTeamDetails) {
              ...
              const updateAssignedModulesResponse = await axios.put(
                `http://10.8.0.13:5030/update-assigned-modules/${selectedTeamDetails.teamId}`,
             ...
             // console.log("Update Assigned Modules Response:", updateAssignedModulesResponse.data);
              ...
              const appendTeamsResponse = await axios.put(
                `http://10.8.0.13:5030/append-teams/${details.moduleId}`, 
                { teamIds: [selectedTeamDetails.teamId] }
              );
              const appendworkspaceResponse = await axios.put(
                `http://10.8.0.13:5030/append-workspaceIds/${details.moduleId}`, 
  ...
  const handleCircleClick = async (details) => {
    ...
    try {
      ...
      if (isP001Allowed) {
        ...
        teamsEndpoint = `http://10.8.0.13:5030/get-org-teams/${orgId}`;
        ...
        teamsEndpoint = 'http://10.8.0.13:5030/get-all-teams';
        const teamsResponse = await axios.get(teamsEndpoint);
        setTeamResponse(teamsResponse)
      } else {
      ...
      if (teamsResponse.data && teamsResponse.data.length > 0) {
        ...
        if (selectedTeam) {
          ...
          if (selectedTeamDetails) {
            ...
            // Make the API call to /create-workspace with the payload
            const createWorkspaceResponse = await axios.post('http://10.8.0.13:5030/create-workspace', createWorkspacePayload);
