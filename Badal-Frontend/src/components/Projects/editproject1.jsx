import { Box, Grid, MenuItem, Select, TextField, Backdrop, CircularProgress, Typography,Card,FormControl,
...
useEffect(() => {
  const fetchProjectData = async () => {
    try {
      const projectRes = await axios.get(`http://10.8.0.13:5030/get-project/${projectId}`);
      setUsefulData({
        ...projectRes.data,
...
const handeleEditProject = async () => {
  ...
  try {
    ...
    const gitlabProjectRes = await axios.put(`http://10.8.0.13:5030/edit-group-description/${gitlabId }`, {
      //name: projectName,
      newDescription: `System Generated - This is a PROJECT created in Badal with the name [**${projectName}**](http://10.8.0.13:3000/project)\n \n \n${projectDescription}`
    });
    console.log(gitlabProjectRes.data);
    const gitlabProjectWebUrl = gitlabProjectRes.data.web_url;

    // Update project in local database with GitLab web URL
    const updateProjectDbRes = await axios.put(`http://10.8.0.13:5030/edit-project-DB/${projectId}`, 
      usefulData
    );
    console.log(updateProjectDbRes.data);
