...
// const gitlabUrl = process.env.gitlabUrl
const accessToken = 'glpat-xsKrrCzsMWsHxTA6xy4o'
const gitlabUrl = 'https://pl-git.iiit.ac.in/api/v4' 
const fs = require('fs');
...
const subgroupGitId= process.env.GIT_ID 
console.log(subgroupGitId);
...
router.delete('/delete-subgroup/:id', (req, res) => {
  const subgroupId = req.params.id;

  // Delete the GitLab subgroup with the specified ID
  axios.delete(`${gitlabUrl}/groups/${subgroupId}`, {
    headers: {
      'PRIVATE-TOKEN': accessToken,
      'Content-Type': 'application/json',
    },
...
//   const { groupName, userName, email, password } = req.body;
...
//       parent_id: subgroupId,
//       visibility: 'private',
//       description: `Description for ${groupName}` // You can customize the description as needed
//     }, {
//       headers: {
//         'PRIVATE-TOKEN': accessToken
//       }
...
//     const newSubgroup = createSubgroupResponse.data;
...
//       projects_limit: 0 // Optional: Set the limit of projects the user can create
//     }, {
//       headers: {
//         'PRIVATE-TOKEN': accessToken
//       }
...
//     // Assign the user to the subgroup
//     await axios.post(`${gitlabUrl}/groups/${newSubgroup.id}/members`, {
//       user_id: newUser.id,
//       access_level: 40 // Provide the appropriate access level (e.g., 30 for Developer)
//     }, {
//       headers: {
//         'PRIVATE-TOKEN': accessToken
//       }
...
// });
router.post('/create-org-subgroup', async (req, res) => {
  const parentGroupId=subgroupGitId
  const { groupName, userName, email, password } = req.body;

  try {
    // Step 1: Create the GitLab subgroup under the specified parent group
    const createSubgroupResponse = await axios.post(`${gitlabUrl}/groups`, {
      name: groupName,
      path: groupName.toLowerCase(),
    ...
    // Step 2: Create a new GitLab user
    const createUserResponse = await axios.post(`${gitlabUrl}/users`, {
      email,
      username: userName,
      name: userName,
      password,
      skip_confirmation: true,
      projects_limit: 0 // Optional: Set project creation limit for the user
    }, {
      headers: {
        'PRIVATE-TOKEN': accessToken
      }
    ...
    // Step 3: Assign the user to the newly created subgroup with Maintainer access
    await axios.post(`${gitlabUrl}/groups/${newSubgroup.id}/members`, {
      user_id: newUser.id,
      access_level: 40 // Maintainer access level
    }, {
      headers: {
        'PRIVATE-TOKEN': accessToken
      }
    ...
    // Respond with the IDs of the created subgroup and user
    res.json({
      subgroupId: newSubgroup.id,
      userGitLabId: newUser.id,
      gitlabUrl:newSubgroup.web_url,
    });
  } catch (error) {
...
router.put('/update-subgroup/:subgroupId', async (req, res) => {
  const subgroupId = req.params.subgroupId;
  const { name, description, userIds } = req.body;

  try {
    // Update the GitLab subgroup details (name and description)
    if (name || description) {
      await axios.put(`${gitlabUrl}/groups/${subgroupId}`, {
        name,
        path: name ? name.toLowerCase() : undefined, // Update path if name is provided
        description
      }, {
        headers: {
          'PRIVATE-TOKEN': accessToken
    ...
    // Fetch existing members in the GitLab subgroup
    const existingMembersResponse = await axios.get(`${gitlabUrl}/groups/${subgroupId}/members`, {
      headers: {
        'PRIVATE-TOKEN': accessToken
      }
    });
    ...
    const usersToRemove = existingMembers.filter(member => !userIds.includes(member.id));

    // Identify users to be added
    const usersToAdd = userIds.filter(userId => !existingMembers.some(member => member.id === userId));

    // Remove existing members from the subgroup
    const removeUserPromises = usersToRemove.map(async (user) => {
      try {
        await axios.delete(`${gitlabUrl}/groups/${subgroupId}/members/${user.id}`, {
          headers: {
            'PRIVATE-TOKEN': accessToken
          }
        });
      } catch (error) {
        console.error(`Error removing user ${user.id} from subgroup:`, error.message);
      }
    });
    ...
    // Add new users to the subgroup
    const addUserPromises = usersToAdd.map(async (userId) => {
      try {
        await axios.post(`${gitlabUrl}/groups/${subgroupId}/members`, {
          user_id: userId,
          access_level: 30 // Access level: 30 provides Developer access. Adjust as needed.
    ...
    res.json({ message: 'Subgroup details and users updated successfully' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
// }); 
router.post('/fork-and-add-to-subgroup/:repoGitlabId/:subgroupId', async (req, res) => {
  const { repoGitlabId, subgroupId } = req.params;
  //const { gitlabUrl, accessToken } = req.body; // Assuming you're passing gitlabUrl and accessToken in the request body
  ...
  try {
    ...
    // Add the forked repository to the subgroup
    await axios.post(`${gitlabUrl}/groups/${subgroupId}/projects/${forkedRepoId}`, null, {
      headers: {
        'PRIVATE-TOKEN': accessToken
      }
    });

    // Optionally, export issues from the original repository
    const exportIssuesResponse = await axios.get(`${gitlabUrl}/projects/${repoGitlabId}/issues`, {
      headers: {
        'PRIVATE-TOKEN': accessToken
      }
    });

    const issuesToImport = exportIssuesResponse.data;

    // Import issues into the forked repository
    for (const issue of issuesToImport) {
      const { title, description, labels, ...otherFields } = issue; // Customize this based on your issue schema
      await axios.post(`${gitlabUrl}/projects/${forkedRepoId}/issues`, {
        title,
        description,
        labels,
        ...otherFields,
