    ...
    try {
        ...
        if (rolesCount === 0 && permissionsCount === 0 && rolePermissionsCount === 0 && permissionsExtraCount === 0) {
            ...
            await Permission.insertMany([
                ...
                { permissionId: 'P006', permissionName: 'Core', description: 'Allows Breaking modules' },
            ]);

            await RolePermission.insertMany([
                { roleId: 'R001', permissionId: ['P005'] },
                { roleId: 'R002', permissionId: ['P001', 'P002', 'P003', 'P004'] },
                { roleId: 'R002-A', permissionId: ['P001', 'P002', 'P003', 'P004', 'P006'] },
                { roleId: 'R002-B', permissionId: ['P001', 'P002', 'P003', 'P004'] },
                { roleId: 'R003', permissionId: ['P002'] },
                { roleId: 'R003-A', permissionId: ['P002', 'P006'] },
        ...
        if (usersCount === 0) {
            const initialUsers = [
                {
                    username: 'ADMIN',
                    password: await bcrypt.hash('rcts', 10),
                    email: 'rcts@gmail.com',
                    roleId: 'R001',
                },
                {
                    username: 'core',
...
//     }
// });
const openCageApiKey = 'd41367ba67074f9280963f9e74127f2d';
...
router.post('/org-signup', async (req, res) => {
    try {
        ...
        const fullAddress = `${Address1}, ${Address2}, ${AreaName}, ${City}, ${state}, ${country}, ${pincode}`;
        ...
        res.json({
            ...
            email: organization.email,
            roleID: organization.roleId,
            gitlabId: organization.gitlabId ,
            gitlabUrl:organization.gitlabUrl,
        });
    } catch (error) {
        console.error(error);
        res.status(500).send('Internal Server Error');
